...
def test_week_2():
    ...
    assert hw_w2[2] == "do homework for week 2 including book exercises 1.31(a), 1.32(a), 1.33, 1.40, 1.41, 1.43, 1.46"
...
def test_file_exists():
    for week, dictie in all_weeks.items():
        if 'Reading_extra' in dictie:
            title, path = dictie['Reading_extra']['title'], dictie['Reading_extra'][
                'path']
            w = Path(__file__).parent.parent
            c = w / path
            assert c.exists()
