...
(require (rename-in "base-eval.rkt"
                    [eval mc-eval]
                    [apply mc-apply])
         "stream.rkt")
          ...
          (let ([instance (instantiate query-pattern
            ...
            (if (history-ref instance)
                ...
                (begin
                  ...
                  ;; the evaluted rule body is returned, but the instantiated query needs to be removed from history
                  ;; so there are no extraneous queries left in the history
                  (let ((evaluated-rule
                         (qeval (rule-body clean-rule)
                                (singleton-stream unify-result))))
                    ;; if the query is left in the history now, earlier calls in the recursive call stack couldn't create an identical query later.
                    ;; :TODO add a history argument to replace the global mutable history.
