...
(define (apply procedure arguments env)
  (when tracing
    (printf "~napply~n~a" procedure)) 
  (cond ((primitive-procedure? procedure)
         (apply-primitive-procedure 
          procedure 
          (list-of-arg-values arguments env)))  
        ((or (compound-procedure? procedure)  
             (list-procedure? procedure))     ; lazy lists are also delayed
         (eval-sequence
...
;; Thunks should be memoised so that once a value has been forced
...
; replace 'OK with racket's (void) to suppress output
; when using eval-assignment in the library
...
;; lazy list printing 
;; lazy lists are just cons cell implemented as lambda expressions
(define (eval-lazy-list exp env) 
  (make-list-procedure(lambda-parameters exp)
                      (lambda-body exp)
                  env))

;; create a new lazy list - use the tag 'list-procedure
;; to identify lazy lists
(define (make-list-procedure parameters body env)
  (mlist 'list-procedure parameters body env))
...
(define (install-syntax)
  ...
  (put-syntax! 'lazy-list eval-lazy-list)     ; lazy lists
  (void))
(install-syntax)
...
;(interpret '(define (cons x y)
;              (cons (m) (m x y))))
;; instead of a regular lambda create a new lazy-list primitive
;; to identify them separately from true lambda promitives
(interpret '(define (cons x y)
              (lazy-list (m) (m x y))))
(interpret '(define (car z)
              (z (lambda (p q) p))))
(interpret '(define (cdr z)
              (z (lambda (p q) q))))
(interpret '(define (list-ref items n)
              (if (= n 0)
                  (car items)
                  (list-ref (cdr items) (- n 1)))))
(interpret '(define (map proc items)
              (if (null? items)
                  '()
                  (cons (proc (car items))
                        (map proc (cdr items))))))
(interpret '(define max-display-items 5))
(interpret '(define (display-lazy-list items)
              (define (show items count)
                (cond ((null? items) (display ""))
                      ...
                      (else (display (car items))
                            (if (null? (cdr items))
                                (display "")
                                (display " "))
                            (show (cdr items) (- count 1)))))
              (display "(")
              (show items max-display-items)
              (display ")")))
...
(define (apply-primitive-procedure proc args)
  (apply-in-underlying-scheme
   ; potentially infinite streams/lists - so don't convert deeply nested
   ; values - only convert the outer layer
...
(define (driver-loop)
  ...
  (let ((input (read)))
    (let ((output
           (actual-value (exp->mlist input) the-global-environment)))
...
(define (display-lazy-list object)
  (eval (mlist 'display-lazy-list object)
